...
int main(int argc, char* argv[])
{
	...
	Long64_t Yb152_tmin = 3000;
	Long64_t Yb152_tmax = HalfLife*4;

	const	Int_t ng = 5;
	Long64_t Emin[ng] =  {1529, 357, 310, 346, 137};
	Long64_t Emax[ng] =  {1535, 361, 315, 350, 142};
	Long64_t BLmin[ng] = {1520, 352, 300, 342, 133};
	Long64_t BLmax[ng] = {1525, 354, 305, 344, 135};
	Long64_t BRmin[ng] = {1537,   0, 320, 352, 144};
	Long64_t BRmax[ng] = {1542,   0, 325, 354, 147};
	...
	for(int i=0; i<ng; i++)
	{
		hdta[i] = new TH1D(Form("hdta_%d", int(Eval[i])), Form("hdta_%d", int(Eval[i])), 100, -300.0, 300.0);
		hdtb[i] = new TH1D(Form("hdtb_%d", int(Eval[i])), Form("hdtb_%d", int(Eval[i])), 100, -300.0, 300.0);
		hdt[i] = new TH1D(Form("hdt_%d", int(Eval[i])), Form("hdt_%d", int(Eval[i])), 100, -300.0, 300.0);
	...
	for(ULong64_t i=0; i < nall; i++)
	{
		...
		{
			if((*rPxts)[j]- rts > Yb152_tmin && (*rPxts)[j]- rts < Yb152_tmax)
			{
				hPGeXa->Fill((*rPxE)[j]);
			...
			if((*rPxts)[j]- rts > -1*Yb152_tmax && (*rPxts)[j]- rts < -1*Yb152_tmin)
			{
				hPGeXb->Fill((*rPxE)[j]);
				for(int k=0; k<Int_t((*rPxE).size()); k++)
				{
		...
		for(int j=0; j<Int_t((*rCe).size()); j++)
		{
			...
			if((*rCts)[j]- rts > Yb152_tmin && (*rCts)[j]- rts < Yb152_tmax)
			{
				//Clover
				hCa->Fill((*rCe)[j]);
			...
			if((*rCts)[j]- rts > -1*Yb152_tmax && (*rCts)[j]- rts < -1*Yb152_tmin)
			{
				//Clover
				hCb->Fill((*rCe)[j]);
				for(int k=0; k<Int_t((*rCe).size()); k++)
