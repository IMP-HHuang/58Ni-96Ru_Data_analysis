...
//		if(jentry > 1E6)	break;
		...
		//PGe Y 
		for(int i=0; i<PGeyEvent->size(); i++)
		{
			if(PGeyEvent->at(i).e < 10 || PGeyEvent->at(i).e > 480)	continue;
			if(PGeyEvent->at(i).id==0 || PGeyEvent->at(i).id == 11) continue;
			Data pgeyTmp;
			InitEnergy(&pgeyTmp);
			pgeyTmp.e = PGeyEvent->at(i).e;
			pgeyTmp.id = PGeyEvent->at(i).id;
			pgeyTmp.t = PGeyEvent->at(i).t;
			auto iPTmp = mPGeY.find(pgeyTmp.t/DeltaT);
			if(iPTmp == mPGeY.end())
			{
				mData mDataTmp;
				mDataTmp.clear();
				mDataTmp.insert(std::pair<Long64_t, Data>(pgeyTmp.t, pgeyTmp));
				mPGeY.insert(std::pair<Long64_t, mData>(pgeyTmp.t/DeltaT, mDataTmp));
			}
			else
				iPTmp ->second.insert(std::pair<Long64_t, Data>(pgeyTmp.t, pgeyTmp));
		}
		xv.clear();
...
void tree::Loop(TTree *opt_, Int_t run_)
{
	...
	{//recoil
		...
		*/
		// PGEY
		auto iP11 = mPGeY.lower_bound((ir->first - DelayedGammaTimeMin * us)/DeltaT);
		for(auto iP1 = iP11; iP1 != mPGeY.end(); iP1++)
		{
			if(iP1->first > ((ir->first - DelayedGammaTimeMin * us)/DeltaT + 1))
				break;
			auto iP22 = iP1->second.lower_bound(ir->first - DelayedGammaTimeMin * us);
			for(auto iP2 = iP22; iP2 != iP1->second.end(); iP2++)
			{
				if (iP2->first >= ir->first + DelayedGammaTimeMax * us)
					break;
				rPgeYE.push_back(iP2->second.e);
				rPgeYid.push_back(iP2->second.id);
				rPgeYts.push_back(iP2->second.t);
			}
		}
